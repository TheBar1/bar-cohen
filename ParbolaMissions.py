...
def main():
    bestX = 10 #יכיל את מספר הנקודות המקסימלי
    points = ListPoints()#מכיל את מטריצת הנקודות
    for i in range (50):
        a, b, c = InputABC() 
        x, d = DistanceB(a, b, c, points)  #x = למספר הנקודות שלא על הפרבולה    d = לסכום המרחקים של הנקודות מהפרבולה
        if x == 0 and d == 0:
            bestA = a
            bestB = b
            bestC = c
